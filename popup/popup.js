...
function renderScheduleTable(schedule) {
	...
	schedule.forEach((entry, i) => {
      ...
      <td class="block2-cell">
        <input type="text" class="time-input" data-i="${i}" data-block="1" data-field="start" value="${b1.start === '00:00' ? '' : b1.start}" placeholder="00:00" maxlength="5" />
        <input type="text" class="time-input" data-i="${i}" data-block="1" data-field="end"   value="${b1.end === '00:00' ? '' : b1.end}"   placeholder="00:00" maxlength="5" />
...
async function checkActiveTab() {
	...
	const isBamboo = tab?.url?.includes('bamboohr.com/employees/timesheet');
	const btn = document.getElementById('open-preview');
	const status = document.getElementById('page-status');

	btn.disabled = !isBamboo;
	if (!isBamboo) {
		setStatus(status, 'Navigate to a BambooHR Timesheet page to enable filling.', 'error');
	} else {
		setStatus(status, 'Timesheet page detected.', 'success');
	}
}
...
async function init() {
	...
	document.getElementById('save-schedule').addEventListener('click', async () => {
		const invalid = [...document.querySelectorAll('input.time-input')].filter((i) => i.value && !/^([01]\d|2[0-3]):[0-5]\d$/.test(i.value));

		if (invalid.length) {
			invalid.forEach((i) => (i.style.borderColor = '#f01800'));
		...
		const updated = readScheduleFromForm(
			schedule.map((e) => ({
				...e,
	...
	document.getElementById('reset-schedule').addEventListener('click', () => {
		renderScheduleTable(DEFAULT_SCHEDULE.map((e) => ({ ...e, blocks: e.blocks.map((b) => ({ ...b })) })));
		setStatus(document.getElementById('save-status'), 'Reset to default — click Save to apply.', 'info');
	});
