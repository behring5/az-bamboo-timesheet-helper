...
function showPreview(newEntries, existingDates, holidayDates) {
  return new Promise((resolve) => {
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      if (!entries.length) { close(null); return; }
...
function buildModalHTML(newEntries, existingDates, holidayDates, allDates) {
  ...
  const rows = allDates.map(date => {
    ...
    return dayEntries.map((entry, blockIndex) => {
      const globalIndex = newEntries.indexOf(entry);
      const dateCell = blockIndex === 0
        ? `<td rowspan="${dayEntries.length}">${label}</td>`
        : '';
      const checkCell = blockIndex === 0
        ? `<td rowspan="${dayEntries.length}"><input type="checkbox" class="bhr-row-check" checked title="Include this day" /></td>`
        : '';
